    ...
    public void run() {
        try {
            ...
            while(true) {
                ...
                field.canMove(this);
                //
                if((x+speedX >= rectangle.getX()) && (x+speedX <= rectangle.getWidth()+rectangle.getX()) && (y+speedY <= rectangle.getY()))
                    if(y + speedY >= rectangle.getY()-radius){
                        // Достигли верхней плоскотси прямоугольника, отскакиваемся вверх
                        speedY = -speedY;
                        y = rectangle.getY()-radius;
                    }

                if((y + speedY >= rectangle.getY()) && (y+ speedY <= rectangle.getHeight()+rectangle.getY()) && (x+speedX <= rectangle.getX()))
                    if((x + speedX>= rectangle.getX()-radius)){
                        // Достигли правой плоскотси прямоугольника, отскакиваемся влево
                        speedX = -speedX;
                        x = rectangle.getX()-radius;
                    }

                if((x+speedX >= rectangle.getX()) && (x+speedX <= rectangle.getWidth()+rectangle.getX()) && (y+speedY >= rectangle.getY()+rectangle.getHeight()))
                    if((y + speedY<= rectangle.getY()+rectangle.getHeight()+radius)){
                        // Достигли нижней плоскотси прямоугольника, отскакиваемся вниз
                        speedY = -speedY;
                        y = rectangle.getY()+rectangle.getHeight()+radius;
                    }

                if((y + speedY >= rectangle.getY()) && (y+ speedY <= rectangle.getHeight()+rectangle.getY()) && x+speedX >= rectangle.getX()+rectangle.getWidth())
                    if((x + speedX<= rectangle.getX()+rectangle.getWidth()+radius)){
                        // Достигли левой плоскотси прямоугольника, отталкиваемся вправо
                        speedX = -speedX;
                        x = rectangle.getX()+rectangle.getWidth()+radius;
                    }
                //
                if (x + speedX <= radius) {
